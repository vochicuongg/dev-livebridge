...
class RulesNotificationDedupScreen extends StatefulWidget {
  const RulesNotificationDedupScreen({super.key});

  @override
  State<RulesNotificationDedupScreen> createState() =>
      _RulesNotificationDedupScreenState();
}

class _RulesNotificationDedupScreenState
    extends State<RulesNotificationDedupScreen> {
  bool _dedupEnabled = false;
  NotificationDedupMode _dedupMode = NotificationDedupMode.otpStatus;
  ...
  Future<void> _loadPrimaryState() async {
    try {
      final Future<bool> dedupEnabledFuture =
          LiveBridgePlatform.getNotificationDedupEnabled();
      final Future<String> dedupModeFuture =
      ...
      final bool dedupEnabled = await dedupEnabledFuture;
      final NotificationDedupMode dedupMode = NotificationDedupModeId.from(
        await dedupModeFuture,
      );
  ...
  Future<void> _setDedupEnabled(bool value) async {
    if (value == _dedupEnabled) {
      return;
    }
    setState(() => _dedupEnabled = value);
    await LiveBridgePlatform.setNotificationDedupEnabled(value);
  }

  Future<void> _setDedupMode(NotificationDedupMode value) async {
    if (value == _dedupMode) {
      return;
    }
    setState(() => _dedupMode = value);
  ...
  Widget build(BuildContext context) {
    ...
    final List<LbListItemData> dedupItems = <LbListItemData>[
      LbListItemData(
        ...
        onToggle: (bool value) {
          unawaited(_setDedupEnabled(value));
        },
        onTap: () {
          final bool nextValue = !_dedupEnabled;
          unawaited(LiveBridgeHaptics.toggle(nextValue));
          unawaited(_setDedupEnabled(nextValue));
        },
      ),
      LbListItemData(
        title: strings.notificationDedupModeLabel,
        subtitle: _dedupMode == NotificationDedupMode.otpOnly
            ? strings.notificationDedupModeOtpOnly
            : strings.notificationDedupModeOtpStatus,
        enabled: _dedupEnabled,
        onTap: _dedupEnabled
            ? () {
                final NotificationDedupMode nextValue =
                    _dedupMode == NotificationDedupMode.otpStatus
                    ? NotificationDedupMode.otpOnly
                    : NotificationDedupMode.otpStatus;
                unawaited(LiveBridgeHaptics.selection());
                unawaited(_setDedupMode(nextValue));
              }
            : null,
      ),
    ];
    ...
    return LbDetailScreen(
      title: strings.notificationDedupTitle,
      floatingBottom: !_dedupEnabled || _isAppsLoading
          ? null
          : LbSearchPill(
      ...
      floatingBottomReservedHeight: !_dedupEnabled || _isAppsLoading
          ? 0
          : LbSpacing.searchPillReservedHeight,
      children: <Widget>[
        LbListComponent(items: dedupItems, extendDividersToEnd: true),
        const SizedBox(height: LbSpacing.detailSectionGap),
        AnimatedOpacity(
          duration: const Duration(milliseconds: 180),
          opacity: _dedupEnabled ? 1 : 0.4,
          child: IgnorePointer(
            ignoring: !_dedupEnabled,
            child: Column(
