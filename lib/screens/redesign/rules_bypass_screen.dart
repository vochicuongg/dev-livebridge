...
class RulesBypassScreen extends StatefulWidget {
  const RulesBypassScreen({super.key});

  @override
  State<RulesBypassScreen> createState() => _RulesBypassScreenState();
}

class _RulesBypassScreenState extends State<RulesBypassScreen> {
  ...
  Future<void> _loadPrimaryState() async {
    try {
      final String packageRules =
          await LiveBridgePlatform.getBypassPackageRules();
      if (!mounted) {
        return;
      }
      setState(() => _selectedPackages = lbParsePackageRules(packageRules));
    } catch (_) {}
  }
  ...
  Future<void> _setAppEnabled(String packageName, bool value) async {
    ...
    await LiveBridgePlatform.setBypassPackageRules(lbEncodePackageRules(next));
  }
